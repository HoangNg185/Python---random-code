...
df=pd.read_csv('C:\\Users\\Liam\\OneDrive - Seneca\\Desktop\\Leaning SQL\\kaggle\\stores_sales_forecasting.csv', encoding='latin1')
...
for i in newnew.index:
    ...
    newnew.loc[i,'City']=newnew.loc[i,'City']+ '-'+newnew.loc[i,'State']+ '-'+newnew.loc[i,'Region']+ '-'+str(newnew.loc[i,'Postal Code'])
...
newnew=newnew.drop(columns=['new','Ship Mode','Customer ID','Sub-Category','Quantity','State','Region','Segment','Postal Code','Product ID','Ship Date','Order Date','Discount'])
